...
def dive():
  ...
  import cv2
  import random
  img1 = cv2.imread('volleyball_pic2.jpg')
  ...
  with mp_pose.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
   ...
   # hand
            ...
            if newx > 400*4//5 and (pixelL and pixelR and left_knee) and good == 0 and touched==0 and ( (pixelL[1]>=newx and pixelL[1]<=newx+(rows) and pixelL[0] >= y and pixelL[0] <= y+(cols)) or (pixelR[1]>=newx and pixelR[1]<=newx+(rows) and pixelR[0] >= y and pixelR[0] <= y+(cols)) )  and cnt >= 30 and (noseNormalize[1] > 400*4//5) :
                touched+=1
                score+=1
                print("score",score)
                cv2.putText(image, 'Score', (200, 250), cv2.FONT_HERSHEY_DUPLEX,2.5, (0, 255, 255), 1, cv2.LINE_AA)
            ...
            if  newx > 0 and newx < 400 and cnt >= 30: #cnt > 30 and cnt < 100  and
                ...
                cv2.circle(image,(y, x), 15, (255, 0, 0), -1)
