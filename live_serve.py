...
def dect_loop(Left):
  ...
  try:
    ...
    while(True):
           ...
           if numFrame == 1 or head==0 or shoulder == 0:
           ...
           if len(pose) == 30:
               ...
               with tf.compat.v1.Session() as sess:
                   new_saver = tf.compat.v1.train.import_meta_graph('lstm/servelog1/serve_30_multiplayer.ckpt.meta')
                   new_saver.restore(sess,'lstm/servelog1/serve_30_multiplayer.ckpt')
           ...
           if Left == 1:
              ...
              datum.cvOutputData=cv2.flip(frame,1) 
              cv2.imshow("OpenPose 1.7.0 - Tutorial Python API", datum.cvOutputData)
           if cv2.waitKey(1) & 0xFF == ord('q'):break
           if number == 45 or wait==100:
