...
try:
        ...
        import Physical.record_volleyball
        import Physical.angle
except OSError as e:
    print(e)    
...
def total_init():
    ...
    img=PIL.Image.open("pic/ssa.png")
    ...
    file = open("Physical/timestamp.txt", "a+")
    file = open("Physical/timestamp.txt", "r")
    ...
    file_name='Physical/timestamp.txt'
    when_month=1
    when_year=2021
    goaltime=0
    action='toss'
    ...
    # 创建一个按钮,并把上面那个函数绑定过来
    button0=Button(master=self_train, text="  OK  ", font=("Arial", 20), bg='#14A769', fg='white',  command=lambda:[sure(),_quit(),Physical.record_volleyball.total_draw(file_name,end,time_choose2,when_year,when_month)])#, command=lambda:[sure,_quit]
    ...
    button_angle=Button(master=self_train, text="  OK  ", font=("Arial", 15), bg='#14A769', fg='white',  command=lambda:[_quit_angle(),Physical.angle.training(action,goaltime)])#
    button_angle.pack_forget()
