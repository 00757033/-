  ...
  with mp_pose.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    while cap.isOpened():
        ...
        try:
            ...
            mouthWH = (math.hypot(left_mouth[0]*100 - right_mouth[0]*100, left_mouth[1]*100 - right_mouth[1]*100))
            #print("mouth", mouthWH)
            #print("left", left_mouth)
            if cnt == 1 :
                nose_ball = cv2.resize(img1, (int(math.sqrt(mouthWH*2500)), int(math.sqrt(mouthWH*2500))))
            #cv2.imshow("Ball", nose_ball)
            #print("int(mouthWH*1000)",int(math.sqrt(mouthWH*2500)))
            #rows,cols = img1.shape[:2]
            rows, cols = nose_ball.shape[:2]
            
            if cnt == 30 :
                #x = 1#random.randint(1,400)
                y = random.randint(1,650-int(math.sqrt(mouthWH*2500)))
                newx = 1
            ...
            if  newx > 0 and newx < 400 and cnt >= 30: #cnt > 30 and cnt < 100  and
                ...
                img1gray = cv2.cvtColor(nose_ball,cv2.COLOR_BGR2GRAY)
                ...
                img1_fg = cv2.bitwise_and(nose_ball,nose_ball,mask = mask_inv)
            ...
            cv2.putText(image, "Score: {}".format(score), (20, 55), cv2.FONT_HERSHEY_DUPLEX,1.5, (0, 255, 255), 1, cv2.LINE_AA)
            ...
            if abs(pixelL_thumb[0]-pixelR_thumb[0]) < 40 and abs(pixelL_thumb[1]-pixelR_thumb[1]) < 15 and angleL_hand > 90 and angleR_hand > 90 and pixelL_thumb[1] < noseNormalize[1]-50 and score > 0 and ball > 0:
                ...
                cv2.putText(image, "You get {} out of {}.  {}%".format(score, ball, round(score*100/ball)) , (25, 300), cv2.FONT_HERSHEY_DUPLEX,1.25, (0, 255, 255), 1, cv2.LINE_AA)
                ending_count += 1
