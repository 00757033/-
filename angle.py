        ...
        while cap.isOpened():
            ...
            try:
                ...
                if action=="serve"and check==1 and count321==1:
                    ...
                    if serve_righthand_higherthan_ear>0   and serve_righthand_elbow_angle >150 and serve_lefthand_higherthan_ear>0   and serve_stage =='step1/3':#拋球 兩手皆要超過耳朵
                        count=40
                        serve_stage="step2/3"
                        print("step2/2")
                        print(count)
        
                    if  serve_lefthand_higherthan_ear<0  and serve_lefthand_elbow_angle >150  and serve_stage =='step2/3':
                        serve_stage="step3/3"
                        counter_serve +=1
                        count=-1
                        #print("step3")
                                ...
                                if abs(squat_lefthand_hip_angle - squat_lefthand_knee_angle<20 and lock==0)or (abs(squat_righthand_hip_angle - squat_righthand_knee_angle)<20 and lock==0):
                                    if (left_foot_index[1]-left_heel[1]<abs(left_foot_index[0]-left_heel[0])/2 and lock==0)or (right_foot_index[1]-right_heel[1]<abs(right_foot_index[0]-right_heel[0])/2 and lock==0):
                                       ...
                                       #print(lock)
                                        ...
                                        Thread(speak("Do not lift the back heel"))
                                else:
                                    Thread(speak("The back is as parallel to the calf as possible "))
                            
                            else:
                                Thread(speak("The angle between the back and the floor must be between 50-80"))
                        else:
                            squat_stage = "step1"
                ...
                if action=="stick"and check==1 and (left_knee[1]<1 and left_knee[1]>0)  and (left_hip[1]<1 and left_hip[1]>0) and (left_shoulder[1]<1 and left_shoulder[1]>0) and count321==1:
                            ...
                            if (stick_lefthand_elbow_angle<110 and stick_lefthand_elbow_angle>70 and stick_lock==0) or (stick_righthand_elbow_angle<110 and stick_righthand_elbow_angle>70 and stick_lock==0) :#預備動作
                                ...
                                if (abs(left_shoulder[0]-left_elbow[0])<abs(left_foot_index[1]-left_heel[1]) and stick_lock==0) or (abs(right_shoulder[0]-right_elbow[0])<abs(right_foot_index[1]-right_heel[1]) and stick_lock==0):   
                                    ...
                                    if (stick_lefthand_balance_angle>20  and  stick_lefthand_balance_angle<60 and stick_lock==0) or (stick_righthand_balance_angle>20 and stick_righthand_balance_angle<60 and stick_lock==0)  :#肩膀與腳趾與地板的角度#防站著
                                        ...
                                        if stick_lock==0:
                                            #sThread(speak("Please lift your body up"))
                                            print(':)')
                                else:
                                    if stick_lock==0:
                                ...
                                if stick_lock==0:
                                    #Thread(speak("Toes lower than knees lower than hips"))
                                    print(':)')
                    ...
                    if stick_stage =='good':
                        ...
                        if stick_seconds >= int(your_goal_time):
                            ...
                            if (stick_lefthand_balance_angle_wrong<=5   and  stick_stage =='good') or (stick_righthand_balance_angle_wrong<=5   and stick_stage =='good')or (stick_lefthand_balance_angle_wrong>=80  and  stick_stage =='good') or (stick_righthand_balance_angle_wrong>=80  and  stick_stage =='good'):#是否倒下 或站立 肩膀與腳踝與地板
                                speak("come on")
                                print("bad2",stick_lefthand_balance_angle_wrong)
                                stick_end = time.time()
                                    # Time elapsed
                                stick_seconds = stick_end - stick_start
                                countfault -=1
